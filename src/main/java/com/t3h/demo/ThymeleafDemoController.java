...
public class ThymeleafDemoController {
    ...
    /**
     * FE đẩy JSON person
     * BE nhận vào object person
     * thêm ID mới
     * response person chuyển về JSON
     */
    @PostMapping("/api/person")
    public ResponseEntity<Person> newPerson(@RequestBody Person person) {

        person.setId("B11");
        log.info("newPerson person={}", person);

        //trả person tự động chuyển về JSON(mặc định)
